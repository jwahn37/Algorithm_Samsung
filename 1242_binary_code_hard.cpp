...
void remove_dup(const int height, const int width, const int code_len);
...
//	cout<<"canbecode "<<h<<" "<<w<<" "<<code_map[h][w]<<endl;
	//if(code_map[h][w]!='0' && code_map[h][w]!='X')	return 1;
	if((code_map[h][w]>='1' && code_map[h][w]<='9') 
		|| (code_map[h][w]>='A' && code_map[h][w]<='F'))	return 1;
	else	return 0;
}
			...
			else
			{
				...
				if(ratio_idx >= 4)
				{
					...
					if(val_idx>=LEN_CODE)
					{
						...
						//remove_dup(height, width, (bitidx+code_len*LEN_CODE+3)/4);
						remove_dup(height, width, (bitidx+code_len*LEN_CODE)/4);
						return code_verify();
					}
...
//현재 암호코드로부터 밑에 중복된 코드들은 모두 제거한다.
void remove_dup(const int height, const int width, const int code_len)
{
	int h,w;
	int wcnt;
	//cout<<"DEBUG remove_dup "<<height<<" "<<width<<" "<<code_len<<endl;
	for(h=height+1; h<N; h++)
	{
		wcnt=0;
		//for(w=width; w<width+code_len && w<M; w++)
		for(w=width; w<width+code_len && w<M; w++)
		{
			if(code_map[h][w] == code_map[height][w])
			{
				//code_map[h][w]='X';
				wcnt++;
			}
		/*	else
			{
				cout<<"why diff? "<<code_map[h][w]<<" "<<code_map[height][width]<<endl;
			}
			*/
		}
	//	cout<<"wcnt "<<wcnt<<endl;
		if(wcnt == code_len)
		{
			for(w=width; w<width+code_len; w++)
				code_map[h][w] = 'X';
		}
		//if(wcnt==0)
		else
...
//	while(1);
	...
	for(i=0; i<10; i++)
	{
		...
		//for(j=0; j<3; j++)
		//{
		if(code_ratio[1] % code_ratio_bin[i][1] != 0)	continue;
		if(code_ratio[2] % code_ratio_bin[i][2] != 0)	continue;
		if(code_ratio[3] % code_ratio_bin[i][3] != 0)	continue;
		//}
		//모든 비가 K로 일정한지 확인
...
char extract_codeval(const int height, const int width, const int bitidx)
{
	...
	//대문자들만 있을걸로 예상
	if(code_map[height][width]>='A' && code_map[height][width]<='F')
	{
		codeval = code_map[height][width]-'A' + 10;
		return (codeval>>(3-bitidx)) & 1;
	}
